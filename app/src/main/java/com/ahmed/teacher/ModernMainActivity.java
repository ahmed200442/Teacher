...
public class ModernMainActivity extends MainActivity {
    ...
    private final Runnable poller = new Runnable(){ public void run(){ try{ polish(); }catch(Exception ignored){} ui.postDelayed(this,700); } };
    ...
    private void startPolishLoop(){ ui.removeCallbacks(poller); ui.postDelayed(poller,220); }
    private GradientDrawable bg(int color, int stroke){ GradientDrawable g=new GradientDrawable(); g.setColor(color); g.setCornerRadius(dp(12)); g.setStroke(dp(stroke),Color.rgb(190,200,220)); return g; }
    private void polish(){
    ...
    private void styleBoxes(ArrayList<EditText> edits,boolean weekly,boolean monthly){
        int[] max=weekly?new int[]{5,5,10,5}:new int[]{15,15,15,15}; int i=0;
        for(EditText e:edits){
            e.setTextSize(20); e.setGravity(Gravity.CENTER); e.setTypeface(Typeface.DEFAULT,Typeface.BOLD); e.setPadding(dp(8),dp(7),dp(8),dp(7)); e.setSelectAllOnFocus(false);
            int limit=max[Math.min(i,max.length-1)]; e.setInputType(android.text.InputType.TYPE_CLASS_NUMBER); e.setBackground(bg(Color.WHITE,2)); e.setElevation(dp(3));
            if(e.getParent() instanceof LinearLayout && !hasScoreLabel((ViewGroup)e.getParent(),e)) addLabel((ViewGroup)e.getParent(),e,limit); i++;
        }
    }
    private boolean hasScoreLabel(ViewGroup p,EditText e){
        for(int i=0;i<p.getChildCount();i++){ View c=p.getChildAt(i); if(c instanceof TextView && c!=e){String t=((TextView)c).getText().toString(); if(t.equals("/5")||t.equals("/10")||t.equals("/15")||t.equals("/40")) return true;} } return false;
    }
    private void addLabel(ViewGroup parent,EditText e,int max){
        ...
        wrap.addView(label,new LinearLayout.LayoutParams(-1,dp(24))); wrap.addView(e,new LinearLayout.LayoutParams(-1,dp(54))); wrap.setLayoutParams(old); parent.addView(wrap,idx);
    }
}
