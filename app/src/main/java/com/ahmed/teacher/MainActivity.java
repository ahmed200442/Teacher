...
public class MainActivity extends Activity {
    ...
    private Button btn(String s, View.OnClickListener l) { Button x=new Button(this); x.setText(s); x.setTextSize(15); x.setAllCaps(false); x.setOnClickListener(l); x.setTextColor(Color.rgb(25,45,90)); x.setLayoutParams(new LinearLayout.LayoutParams(-1,dp(54))); return x; }
    ...
    private void home() {
        ...
        root.addView(btn("📊 التقييم الشهري والتقارير",v->monthly()));
    ...
    private void schoolData() {
        ...
        EditText school=field("اسم المدرسة",getPreferences(0).getString("school",""));
        EditText teacher=field("اسم المعلم",getPreferences(0).getString("teacher","Ahmed Mostafa Abonajy"));
        EditText subject=field("المادة",getPreferences(0).getString("subject",""));
        ...
        new AlertDialog.Builder(this).setTitle("🏫 بيانات المدرسة").setView(box)
            .setPositiveButton("حفظ",(d,w)->{getPreferences(0).edit().putString("school",school.getText().toString()).putString("teacher",teacher.getText().toString()).putString("subject",subject.getText().toString()).apply(); Toast.makeText(this,"تم حفظ بيانات المدرسة",Toast.LENGTH_SHORT).show();})
    ...
    private void classes(int g) {
        ...
        for(int i=1;i<=3;i++){ final int c=i; root.addView(btn("الفصل "+c+" • "+(c==cls&&!names.isEmpty()?names.size():50)+" طالب",v->open(c))); }
    ...
    private void open(int c) {
        ...
        root.addView(tv("عدد الطلاب: "+(names.isEmpty()?"لم يتم الاستيراد بعد":names.size()),17));
        ...
        root.addView(btn("📤 تصدير الفصل إلى Excel",v->exportExcel())); root.addView(btn("🖨 طباعة الفصل",v->printReport())); root.addView(btn("⬅ رجوع",v->classes(grade)));
    }
    ...
    private int savedScore(String student,String w,String part,int max) { return getPreferences(0).getInt(key(student,w,part),0); }
    private void saveScore(String student,String w,String part,int value) { getPreferences(0).edit().putInt(key(student,w,part),Math.min(maxFor(part),Math.max(0,value))).apply(); }
    ...
    private boolean isAbsent(String student,String w) { return getPreferences(0).getBoolean(absentKey(student,w),false); }
    private void saveAbsent(String student,String w,boolean absent) { getPreferences(0).edit().putBoolean(absentKey(student,w),absent).apply(); }
    private String lockKey() { return "locked_"+grade+"_"+cls+"_"+week; }
    private boolean isWeekLocked() { return getPreferences(0).getBoolean(lockKey(),false); }
    private void setWeekLocked(boolean locked) { getPreferences(0).edit().putBoolean(lockKey(),locked).apply(); }

    private void assessment() {
        base("الصف "+grade+" - الفصل "+cls); root.addView(tv(week,20));
        root.addView(tv("التقييم الأسبوعي • المجموع /25 تلقائي",16));
        ...
        if(locked) root.addView(btn("🔓 فتح التقييم للتعديل",v->confirmUnlock()));
        else root.addView(btn("🔒 قفل التقييم الأسبوعي",v->confirmLock()));
        if(names.isEmpty()) for(int i=1;i<=50;i++) names.add("طالب "+i);
        root.addView(btn("✓ الحضور افتراضيًا للجميع - ضع علامة ✓ أمام الغائب",v->Toast.makeText(this,"الحضور محفوظ تلقائيًا، ضع ✓ أمام الغائب فقط",Toast.LENGTH_SHORT).show()));
        for(String n:names) addStudent(n,locked);
    ...
    private void confirmLock() {
        new AlertDialog.Builder(this).setTitle("🔒 قفل التقييم الأسبوعي")
            .setMessage("بعد القفل لن تستطيع تعديل درجات هذا الأسبوع حتى تختار فتح التقييم مرة أخرى. هل تريد القفل؟")
            .setNegativeButton("إلغاء",null).setPositiveButton("قفل",(d,w)->{setWeekLocked(true);assessment();Toast.makeText(this,"تم قفل "+week,Toast.LENGTH_SHORT).show();}).show();
    }
    private void confirmUnlock() {
        new AlertDialog.Builder(this).setTitle("🔓 فتح التقييم")
            .setMessage("سيتم السماح بتعديل درجات هذا الأسبوع مرة أخرى.")
            .setNegativeButton("إلغاء",null).setPositiveButton("فتح",(d,w)->{setWeekLocked(false);assessment();Toast.makeText(this,"تم فتح "+week+" للتعديل",Toast.LENGTH_SHORT).show();}).show();
    }
    ...
    private void addStudent(String n,boolean locked) {
        ...
        CheckBox absent=new CheckBox(this); absent.setText("غائب ✓"); absent.setTextSize(13); absent.setChecked(isAbsent(n,week)); absent.setEnabled(!locked); r1.addView(absent); row.addView(r1);
        ...
        TextView total=tv("المجموع\n"+(savedScore(n,week,"hw",5)+savedScore(n,week,"copy",5)+savedScore(n,week,"test",10)+savedScore(n,week,"beh",5))+" /25",12); total.setBackgroundColor(Color.rgb(238,243,250));
        ...
        if(!locked){
            ...
            hw.addTextChangedListener(w); copy.addTextChangedListener(w); test.addTextChangedListener(w); beh.addTextChangedListener(w);
    ...
    private int weeklyTotal(String n){int sum=0;for(String w:weeks)sum+=savedScore(n,w,"hw",5)+savedScore(n,w,"copy",5)+savedScore(n,w,"test",10)+savedScore(n,w,"beh",5);return Math.min(25,sum);}
    ...
    private void monthly(){base("📊 التقييم الشهري والاختبارات");root.addView(tv("الصف "+grade+" - الفصل "+cls,18));root.addView(tv("كل طالب له درجات الشهر الأول والثاني ومتوسط تلقائي",15));if(names.isEmpty())load();for(String n:names)addMonthlyStudent(n);root.addView(btn("📤 تصدير Excel",v->exportExcel()));root.addView(btn("🖨 طباعة",v->printReport()));root.addView(btn("⬅ رجوع",v->open(cls)));}
    private void addMonthlyStudent(String n){
        ...
        LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER);EditText first=edit("الشهر الأول\n/15"),second=edit("الشهر الثاني\n/15");
        String k1="exam1_"+grade+"_"+cls+"_"+n.hashCode(),k2="exam2_"+grade+"_"+cls+"_"+n.hashCode();loadEdit(first,getPreferences(0).getInt(k1,0));loadEdit(second,getPreferences(0).getInt(k2,0));
        ...
        TextWatcher tw=new TextWatcher(){public void beforeTextChanged(CharSequence s,int a,int b,int c){}public void onTextChanged(CharSequence s,int a,int b,int c){int a1=num(first,15),a2=num(second,15);getPreferences(0).edit().putInt(k1,a1).putInt(k2,a2).apply();double av=(a1+a2)/2.0,wk=weeklyAverage(n);avg.setText("متوسط الشهرين\n"+fmt(av)+" /15");weekly.setText("متوسط التقييمات\n"+fmt(wk)+" /25");total.setText("المجموع الكلي\n"+fmt(av+wk)+" /40");}public void afterTextChanged(Editable e){}};first.addTextChangedListener(tw);second.addTextChangedListener(tw);
        double av=(getPreferences(0).getInt(k1,0)+getPreferences(0).getInt(k2,0))/2.0,wk=weeklyAverage(n);avg.setText("متوسط الشهرين\n"+fmt(av)+" /15");weekly.setText("متوسط التقييمات\n"+fmt(wk)+" /25");total.setText("المجموع الكلي\n"+fmt(av+wk)+" /40");root.addView(card);Space gap=new Space(this);gap.setLayoutParams(new LinearLayout.LayoutParams(1,dp(10)));root.addView(gap);
    }
    private double weeklyAverage(String n){int weeksCount=0;double sum=0;for(String w:weeks){int s=savedScore(n,w,"hw",5)+savedScore(n,w,"copy",5)+savedScore(n,w,"test",10)+savedScore(n,w,"beh",5);if(s>0||hasWeekData(n,w)){sum+=s;weeksCount++;}}return weeksCount==0?0:sum/weeksCount;}
    private boolean hasWeekData(String n,String w){return savedScore(n,w,"hw",5)>0||savedScore(n,w,"copy",5)>0||savedScore(n,w,"test",10)>0||savedScore(n,w,"beh",5)>0;}
