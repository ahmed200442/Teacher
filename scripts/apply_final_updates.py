...
# Weekly grading: Homework 5, Notebook 5, Weekly 10, Attendance/Behavior 5.
...
method='''    private void addStudent(String n,boolean locked){
        ...
        LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER);row.setWeightSum(4);
...
t=re.sub(r'private double weeklyAverage\(String n\)\{.*?\n    private String fmt', '''private double weeklyAverage(String n){double sum=0;int count=0;for(String w:weeks){if(hasWeekData(n,w)){sum+=savedScore(n,w,"beh")+savedScore(n,w,"hw")+savedScore(n,w,"copy")+savedScore(n,w,"weekly");count++;}}return count==0?0:sum/count;}
    ...
    private String fmt''',t,count=1,flags=re.S)
pat=r'    private void monthly\(\)\{.*?\n    private void printReport'
method='''    private void monthly(){
    ...
    private void addMonthlyStudent(String n){
        ...
        TextWatcher tw=new TextWatcher(){public void beforeTextChanged(CharSequence s,int x,int y,int z){}public void onTextChanged(CharSequence s,int x,int y,int z){prefs().edit().putInt(examKey("exam1",n),num(a,15)).putInt(examKey("exam2",n),num(b,15)).apply();recalc();}public void afterTextChanged(Editable e){}};a.addTextChangedListener(tw);b.addTextChangedListener(tw);setAutoNext(a,b,15);setAutoNext(b,null,15);recalc();root.addView(card);Space gap=new Space(this);gap.setLayoutParams(new LinearLayout.LayoutParams(1,dp(8)));root.addView(gap);
    }
